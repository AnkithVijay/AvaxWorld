...
contract AvaxWorld is Ownable {
    ...
    uint256 public constant LOTTERY_FEE = 5;
    ...
    function _invest(address _addr, address _ref, uint256 _amount, uint8 plan) private returns (bool){
        ...
        uint256 lotteryFee = _amount.mul(LOTTERY_FEE).div(PERCENTS_DIVIDER);
        _buyTickets(_addr, lotteryFee);
        return true;
    }
    ...
    function withdraw() public {
        ...
        //lottery
        if(investors[msg.sender].lotteryRewards > 0){
            dividendAmount = dividendAmount.add(investors[msg.sender].lotteryRewards);
            investors[msg.sender].lotteryRewards = 0;
        }
	...
	function getUserLotteryReward(address userAddress) public view returns(uint256) {
		return investors[userAddress].lotteryRewards;
	}
    ...
    function _buyTickets(address userAddress, uint256 amount) private {
        ...
        if(totalTickets > tickets && totalTickets.div(1e18) >=1){
